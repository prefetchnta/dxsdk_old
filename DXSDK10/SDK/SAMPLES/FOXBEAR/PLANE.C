...
HPLANE *CreatePlane ( USHORT width, USHORT height, USHORT denom )
{
    ...
    hPlane->hMask = CMemAlloc( num_elems, elem_size );
    ...
    if( hPlane->hMask == NULL )
    {
        MemFree( hPlane );
        ErrorMessage( "hPlane->hMask in CreatePlane" );
    }
    return hPlane;
...
BOOL TilePlane( HPLANE *hPlane, HBITMAPLIST *hTileList, HPOSLIST *posList )
{
    ...
    for( i = 0; i < hPlane->width * hPlane->height; ++i )
    {
        ...
        if( posList[i] >= 0 )
        {
            ...
            hPlane->hMask[i] = hTileList[ posList[i] ].hMask;
        }
        else
        {
            hPlane->hBM[i] = NULL;
    ...
    for( j = y1; j <= y2; ++j )
    {
        for( i = x1; i <= x2; ++i )
        {
            ...
            if( hPlane->hBM[n] != NULL )
            {
                ...
                {
                    rect.dwLeft  = 0;
                    rect.dwRight = rect.dwLeft + C_TILE_W - xmod;
                    point.dwX    = xmod;
                }
                else if( i == x2 )
                {
                    rect.dwLeft  = (i - x1) * C_TILE_W - xmod;
                    rect.dwRight = rect.dwLeft + xmod;
                    point.dwX    = 0;
                } else {
                    rect.dwLeft  = (i - x1) * C_TILE_W - xmod;
                    rect.dwRight = rect.dwLeft + C_TILE_W;
                    point.dwX    = 0;
                }
                ...
                {
                    rect.dwTop    = 0;
                    rect.dwBottom = rect.dwTop + C_TILE_H - ymod;
                    point.dwY     = ymod;
                }
                else if( j == y2 )
                {
                    rect.dwTop    = (j - y1) * C_TILE_H - ymod;
                    rect.dwBottom = rect.dwTop + ymod;
                    point.dwY     = 0;
                } else {
                    rect.dwTop    = (j - y1) * C_TILE_H - ymod;
                    rect.dwBottom = rect.dwTop + C_TILE_H;
                    point.dwY     = 0;
                }
         
                if( (rect.dwTop != rect.dwBottom) && (rect.dwLeft != rect.dwRight) )
                {
                    if( hPlane->hMask[n] == NULL )
                    {
                        gfxRectXfer (
                            hBuffer, 
                            &rect, 
