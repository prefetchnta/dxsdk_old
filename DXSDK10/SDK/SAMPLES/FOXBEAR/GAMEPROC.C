...
HBITMAPLIST *LoadBitmaps( void )
{
    ...
    {
        Msg( "Could not load art file" );
        return NULL;
    }
    hBitmapList = calloc( C_TILETOTAL + C_FBT + C_BBT, sizeof (HBITMAPLIST) );
    
    for( i = 0; i < C_TILETOTAL; ++i )
    {
        sprintf( fileName, "%03u.BMP", i + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "M%03u.BMP", i + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n = C_TILETOTAL;

    for( i = n; i < n + C_FOXWALK; ++i )
    {
        sprintf( fileName, "FW%02uR.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FW%02uRM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXWALK;
    ...
    for( i = n; i < n + C_FOXWALK; ++i )
    {
        sprintf( fileName, "FW%02uL.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FW%02uLM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXWALK;
    ...
    {
        sprintf( fileName, "FR%02uR.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FR%02uRM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXRUN;
    ...
    for( i = n; i < n + C_FOXRUN; ++i )
    {
        sprintf( fileName, "FR%02uL.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FR%02uLM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXRUN;
    ...
    {
        sprintf( fileName, "FS%1uR.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FS%1uRM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXSTILL;
    ...
    for( i = n; i < n + C_FOXSTILL; ++i )
    {
        sprintf( fileName, "FS%1uL.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FS%1uLM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXSTILL;
    ...
    {
        sprintf( fileName, "FK%1uR.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FK%1uRM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXSTUNNED;
    ...
    for( i = n; i < n + C_FOXSTUNNED; ++i )
    {
        sprintf( fileName, "FK%1uL.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FK%1uLM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXSTUNNED;
    ...
    {
        sprintf( fileName, "FC%1uR.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FC%1uRM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXCROUCH;
    ...
    for( i = n; i < n + C_FOXCROUCH; ++i )
    {
        sprintf( fileName, "FC%1uL.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FC%1uLM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXCROUCH;
    ...
    {
        sprintf( fileName, "FCD%1uR.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FCD%1uRM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXSTOP;
    ...
    for( i = n; i < n + C_FOXSTOP; ++i )
    {
        sprintf( fileName, "FCD%1uL.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FCD%1uLM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXSTOP;
    ...
    {
        sprintf( fileName, "FT%1uR.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FT%1uRM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXTHROW;
    ...
    for( i = n; i < n + C_FOXTHROW; ++i )
    {
        sprintf( fileName, "FT%1uL.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FT%1uLM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXTHROW;
    ...
    {
        sprintf( fileName, "FJT%1uR.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FJT%1uRM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXJUMPTHROW;
    ...
    for( i = n; i < n + C_FOXJUMPTHROW; ++i )
    {
        sprintf( fileName, "FJT%1uL.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FJT%1uLM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXJUMPTHROW;
    ...
    {
        sprintf( fileName, "FJ%1uR.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FJ%1uRM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXJUMP;
    ...
    for( i = n; i < n + C_FOXJUMP; ++i )
    {
        sprintf( fileName, "FJ%1uL.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FJ%1uLM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXJUMP;
    ...
    {
        sprintf( fileName, "FCW%02uR.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FCW%02uRM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXCROUCHWALK;
    ...
    for( i = n; i < n + C_FOXCROUCHWALK; ++i )
    {
        sprintf( fileName, "FCW%02uL.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FCW%02uLM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXCROUCHWALK;
    ...
    {
        sprintf( fileName, "FB%02uR.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FB%02uRM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXBLURR;
    ...
    for( i = n; i < n + C_FOXBLURR; ++i )
    {
        sprintf( fileName, "FB%02uL.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "FB%02uLM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_FOXBLURR;
    ...
    {
        sprintf( fileName, "BM%1uL.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "BM%1uLM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_BEARMISS;
    ...
    {
        sprintf( fileName, "BS%02uL.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "BS%02uLM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_BEARSTRIKE;
    ...
    {
        sprintf( fileName, "BW%02uL.BMP", i - n + 1 );
        hBitmapList[i].hBM = gfxLoadBitmap( fileName );

        sprintf( fileName, "BW%02uLM.BMP", i - n + 1 );
        hBitmapList[i].hMask = gfxLoadBitmap( fileName );
    }
    n += C_BEARWALK;
...
void InitFox ( HSPRITE **pphFox, HBITMAPLIST *phBitmapList )
{
    ...
    *pphFox = CreateSprite( C_FBT, startX, startY, boundW, boundH, C_FORE_W * C_TILE_W, C_FORE_H * C_TILE_H, (SHORT) as, TRUE );
    ...
    for( i = 0; i < C_FBT; ++i )
    {
        ...
        size.dwWidth  = w[i];        // THESE NEED
        size.dwHeight = h[i];        // CHANGING !
        point.dwX     = 0;
        point.dwY     = 0;
        ...
        hBM = gfxCreateVramBitmap( &size, GFX_BMF_8BPP );
        hBM_mask = phBitmapList[i + C_TILETOTAL].hMask;

        gfxRectXfer( hBM, &rect, hBM_src, &point, GFX_MIX_S );
...
void InitBear( HSPRITE **pphBear, HBITMAPLIST *phBitmapList )
{
    ...
    *pphBear = CreateSprite( C_BBT, startX, startY, boundW, boundH, C_FORE_W * C_TILE_W, C_FORE_H * C_TILE_H, (SHORT) as, TRUE );
    ...
    for( i = 0; i < C_BBT; ++i )
    {
        ...
        hBM = gfxCreateVramBitmap( &size, GFX_BMF_8BPP );
        hBM_mask = phBitmapList[i + C_TILETOTAL + C_FBT].hMask;
...
VOID InitApple ( HSPRITE **pphApple, HBITMAPLIST *phBitmapList )
{
    ...
    *pphApple = CreateSprite( 1, 50 * C_UNIT, 390 * C_UNIT, 32, 32, C_FORE_W * C_TILE_W, C_FORE_H * C_TILE_H, 0, FALSE );
    ...
    hBM_mask = phBitmapList[61].hMask;

    gfxRectXfer( hBM, &rect, hBM_src, &point, GFX_MIX_S );

    BitBltSprite( *pphApple, hBM, hBM_mask, NONE, RIGHT, 0, 0, 32, 32 );
...
BOOL InitializeGame ( void )
{
    ...
    InitPlane( &hForeground, &hForePosList, "FORELIST.DAT", C_FORE_W, C_FORE_H, C_FORE_DENOM );
    TilePlane( hForeground, hTileList, hForePosList );

    InitPlane( &hMidground, &hMidPosList, "MIDLIST.DAT", C_MID_W, C_MID_H, C_MID_DENOM );
    TilePlane( hMidground, hTileList, hMidPosList );

    InitPlane( &hBackground, &hBackPosList, "BACKLIST.DAT", C_BACK_W, C_BACK_H, C_BACK_DENOM );
    TilePlane( hBackground, hTileList, hBackPosList );

    InitSurface( &hSurfaceList, "SURFLIST.DAT", C_FORE_W, C_FORE_H );
